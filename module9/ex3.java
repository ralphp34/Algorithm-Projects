...
public class ex3 {
  ...
  public static void compute(double arr[], int n, int k, int j, int sum, int m, int[] mp, int mpIdx ){
        // implementation of the base case and storage of result
        if (k == 1) {
            mp[mpIdx] = n-1;
            m = Math.max(m, sum);
            sum = 0;
        ...
        for (int i = j; i < n; i++) {
            ...
            m = Math.max(m, sum);
            mp[mpIdx] = i;
            compute( arr, n, k - 1, i + 1, sum, m, mp, mpIdx + 1 );
        }
  }
  ...
  static int[] myAlgorithm (double[] taskTimes, int numProcessors)
  {
    ...
    partition = new int[numProcessors];
    int[] temp = new int[numProcessors];
    
    result = 1000000;
    
    compute(taskTimes, taskTimes.length, numProcessors, 0,0,0, temp, 0);
    return partition;
  }
